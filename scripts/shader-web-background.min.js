...
(() => {
  ...
  function B(a, b, c) {
    y(
      a instanceof HTMLScriptElement && a.type === b,
      'Shader source element of id "' +
        c +
        '" should be of type: <script type="' +
        (b + '" id="' + c + '">')
    );
  }
  function D(a) {
    ...
    y(
      b,
      'Missing shader source: <script type="x-shader/x-fragment" id="' +
        (a + '">')
    );
  ...
  function E(a) {
    ...
    const b = document.getElementById(a);
    return b
      ? (B(b, "x-shader/x-vertex", a), b.text)
  ...
  class G {
    constructor(a, b, c, d) {
      ...
      const l = a.gl;
      this.h = () => {
        for (const f of d) f.u(l, f.location, b);
      };
      this.i = () => {
        var f = c.v,
          h = a.gl;
        ...
        f = a.gl;
        for (h = 0; h < a.g; h++)
          f.activeTexture(f.TEXTURE0 + h), f.bindTexture(f.TEXTURE_2D, null);
        a.g = 0;
      };
    }
  }
  ...
  function J(a, b, c, d, l, f) {
      ...
      try {
        {
          var k = p;
          const q = k.gl,
            P = K(k, e, q.VERTEX_SHADER, m),
            Q = K(k, e, q.FRAGMENT_SHADER, n),
            v = q.createProgram();
          q.attachShader(v, P);
          q.attachShader(v, Q);
          q.linkProgram(v);
    ...
    for (const e in b) {
      ...
      u = m.m.map((k) => ({ location: k.location, u: n[k.name] }));
